	...
	public function processing() {
		...
		if(!empty($wcfm_coupons_array)) {
			...
			foreach($wcfm_coupons_array as $wcfm_coupons_single) {
				...
				$actions = '';
				if( $wcfm_coupons_single->post_status == 'publish' ) {
				  $actions .= ( current_user_can( 'edit_published_shop_coupons' ) ) ? '<a class="wcfm-action-icon" href="' . get_wcfm_coupons_manage_url($wcfm_coupons_single->ID) . '"><span class="fa fa-edit text_tip" data-tip="' . esc_attr__( 'Edit', $WCFM->text_domain ) . '"></span></a>' : '';
				} else {
					$actions .= ( current_user_can( 'edit_shop_coupons' ) ) ? '<a class="wcfm-action-icon" href="' . get_wcfm_coupons_manage_url($wcfm_coupons_single->ID) . '"><span class="fa fa-edit text_tip" data-tip="' . esc_attr__( 'Edit', $WCFM->text_domain ) . '"></span></a>' : '';
				}
