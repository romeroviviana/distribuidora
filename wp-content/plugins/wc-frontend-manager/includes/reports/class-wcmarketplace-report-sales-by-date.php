...
class WC_Marketplace_Report_Sales_By_Date extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		switch ( $this->chart_groupby ) {
			case 'day' :
				$average_sales_title = sprintf( __( '%s average daily sales', $WCFM->text_domain ), '<strong>' . wc_price( $data->average_sales ) . '</strong>' );
			break;
			case 'month' :
			default :
				$average_sales_title = sprintf( __( '%s average monthly sales', $WCFM->text_domain ), '<strong>' . wc_price( $data->average_sales ) . '</strong>' );
			break;
		}
		
		$legend[] = array(
			'title'            => sprintf( __( '%s total earned commission', $WCFM->text_domain ), '<strong>' . wc_price( $data->total_earned ) . '</strong>' ),
			'placeholder'      => __( 'This is the sum of the earned commission including shipping and taxes if applicable.', $WCFM->text_domain ),
			'color'            => $this->chart_colors['earned'],
			'highlight_series' => 3
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s total paid commission', $WCFM->text_domain ), '<strong>' . wc_price( $data->total_commission ) . '</strong>' ),
			'placeholder'      => __( 'This is the sum of the commission paid including shipping and taxes if applicable.', $WCFM->text_domain ),
		...
		$legend[] = array(
			'title'            => sprintf( __( '%s orders placed', $WCFM->text_domain ), '<strong>' . $data->total_orders . '</strong>' ),
			'color'            => $this->chart_colors['order_count'],
			'highlight_series' => 0
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s charged for shipping', $WCFM->text_domain ), '<strong>' . wc_price( $data->total_shipping ) . '</strong>' ),
	...
	public function get_export_button() {
		...
		<a
			...
			class="export_csv"
			data-export="chart"
			data-xaxes="<?php esc_attr_e( 'Date', $WCFM->text_domain ); ?>"
		...
		>
			<?php esc_html_e( 'Export CSV', $WCFM->text_domain ); ?>
		</a>
		<?php
	}
	...
	public function get_main_chart() {
		...
		$select = "SELECT COUNT( DISTINCT commission.order_id ) AS count, SUM( commission.shipping ) AS total_shipping, SUM( commission.tax ) AS total_tax, SUM( commission.commission_amount ) AS total_commission, commission.created AS time";
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					var series = [
						{
							label: "<?php echo esc_js( __( 'Number of orders', $WCFM->text_domain ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Shipping amount', $WCFM->text_domain ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Average sales amount', $WCFM->text_domain ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Total Commission Earned Amount', $WCFM->text_domain ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Total Commission Paid Amount', $WCFM->text_domain ) ) ?>",
