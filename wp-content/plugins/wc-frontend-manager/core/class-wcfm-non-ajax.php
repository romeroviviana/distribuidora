...
class WCFM_Non_Ajax {
	...
	function wcfm_dashboard_sales_report() {
		...
		if( !empty($top_sellers) ) {
			...
			$top_sellers_array .= '[{label:"' . __( 'Yet to sale your first item ..!!!', $WCFM->text_domain ) . '", data: 1, color: "' . $colors_arr[0] . '"}';
		}
		$top_sellers_array .= ']';
		?>
	...
	public function wcfm_plugin_action_links( $links ) {
		global $WCFM;
		$action_links = array(
			'settings' => '<a href="' . admin_url( 'admin.php?page=wcfm_settings' ) . '" aria-label="' . esc_attr__( 'View WCFM settings', $WCFM->text_domain ) . '">' . esc_html__( 'Settings', $WCFM->text_domain ) . '</a>',
		);
	...
	public function wcfm_plugin_row_meta( $links, $file ) {
		...
		if ( $WCFM->plugin_base_name == $file ) {
			$row_meta = array(
				'docs'    => '<a href="' . esc_url( apply_filters( 'wcfm_docs_url', 'http://wclovers.com/documentation/' ) ) . '" aria-label="' . esc_attr__( 'View WCFM documentation', $WCFM->text_domain ) . '">' . esc_html__( 'Documentation', $WCFM->text_domain ) . '</a>',
				...
				'contactus' => '<a href="' . esc_url( apply_filters( 'wcfm_contactus_url', 'http://wclovers.com/contact-us/' ) ) . '" aria-label="' . esc_attr__( 'Any WC help feel free to contact us', $WCFM->text_domain ) . '">' . esc_html__( 'Contact US', $WCFM->text_domain ) . '</a>'
			);
			
			$ultimate_meta = array();
			if(!WCFM_Dependencies::wcfmu_plugin_active_check()) {
				$ultimate_meta = array( 'ultimate' => '<a href="' . esc_url( apply_filters( 'wcfm_ultimate_url', 'http://wclovers.com/product/woocommerce-frontend-manager-ultimate/' ) ) . '" aria-label="' . esc_attr__( 'Add more power to your WCFM', $WCFM->text_domain ) . '">' . esc_html__( 'WCFM Ultimate', $WCFM->text_domain ) . '</a>' );
			}
	...
	function wcfm_admin_bar_menu() {
				...
				if( !isset( $wcfm_menu_data['capability'] ) || empty( $wcfm_menu_data['capability'] ) || current_user_can( $wcfm_menu_data['capability'] ) ) {
					...
					if( isset( $wcfm_menu_data['has_new'] ) ) {
						...
						$wp_admin_bar->add_menu( array(
							...
							'title' => '<span class="wcfm-admin-menu">' . __( 'Add New', $WCFM->text_domain ) . '</span>',
		...
		/*if( is_admin() ) {
			$wp_admin_bar->add_menu( array(
				...
				'title' => '<span class="wcfm-admin-menu"><span class="fa fa-cog"></span>' . __( 'Settings', $WCFM->text_domain ) . '</span>',
